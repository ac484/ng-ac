...
import { useIndependentSettings } from '../hooks/use-independent-settings';

interface IndependentSidebarSettingsProps {
  className?: string;
}

export function IndependentSidebarSettings({ className }: IndependentSidebarSettingsProps) {
  const { state, updateSidebarVisibility, saveSettings, resetToDefaults } = useIndependentSettings();
    ...
    <Card className={className}>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>側邊欄顯示設置 (獨立模塊)</span>
        </CardTitle>
        <CardDescription>
          控制側邊欄中顯示哪些功能模組和頁面 - 使用獨立的事件驅動系統
        </CardDescription>
      </CardHeader>
