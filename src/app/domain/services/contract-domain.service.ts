...
export class ContractDomainService {
  constructor(
    @Inject(CONTRACT_REPOSITORY_TOKEN) private contractRepository: ContractRepository
  ) {}
  ...
  async generateContractNumber(): Promise<ContractNumber> {
    ...
    const maxSequence = todayContracts.length > 0 
      ? Math.max(...todayContracts.map(c => parseInt(c.contractNumber.getValue().slice(8))))
      : 0;
