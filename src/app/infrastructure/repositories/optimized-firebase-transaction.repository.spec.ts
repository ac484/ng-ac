...
describe('OptimizedFirebaseTransactionRepository', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [OptimizedFirebaseTransactionRepository, { provide: Firestore, useValue: mockFirestore }]
    });

    repository = TestBed.inject(OptimizedFirebaseTransactionRepository);

    // 創建測試用交易
    mockTransaction = OptimizedTransaction.createDeposit('TXN-TEST-001', 'acc_1', 'user_1', new Money(1000, 'USD'), '測試存款');
  });
  ...
  describe('findByTransactionNumber', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByTransactionNumber(transactionNumber)).toBeRejectedWithError(
        RepositoryError,
        '根據交易編號查找交易失敗'
      );
    });
  ...
  describe('findByAccountId', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByAccountId(accountId)).toBeRejectedWithError(RepositoryError, '根據帳戶 ID 查找交易失敗');
    });
  });
  ...
  describe('findByUserId', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByUserId(userId)).toBeRejectedWithError(RepositoryError, '根據用戶 ID 查找交易失敗');
    });
  });
  ...
  describe('findByStatus', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByStatus(status)).toBeRejectedWithError(RepositoryError, '根據交易狀態查找交易失敗');
    });
  });
  ...
  describe('findByType', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByType(transactionType)).toBeRejectedWithError(RepositoryError, '根據交易類型查找交易失敗');
    });
  });
  ...
  describe('findByAmountRange', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByAmountRange(minAmount, maxAmount)).toBeRejectedWithError(
        RepositoryError,
        '根據金額範圍查找交易失敗'
      );
    });
  ...
  describe('findByDateRange', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByDateRange(startDate, endDate)).toBeRejectedWithError(RepositoryError, '根據日期範圍查找交易失敗');
    });
  });
  ...
  describe('findByCategory', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByCategory(category)).toBeRejectedWithError(RepositoryError, '根據分類查找交易失敗');
    });
  });
  ...
  describe('findByReferenceNumber', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.findByReferenceNumber(referenceNumber)).toBeRejectedWithError(
        RepositoryError,
        '根據參考編號查找交易失敗'
      );
    });
  ...
  describe('existsByTransactionNumber', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.existsByTransactionNumber(transactionNumber)).toBeRejectedWithError(
        RepositoryError,
        '檢查交易編號是否存在失敗'
      );
    });
  ...
  describe('getStatistics', () => {
    it('should calculate transaction statistics correctly', async () => {
      // Arrange
      const transaction1 = OptimizedTransaction.createDeposit('TXN-001', 'acc_1', 'user_1', new Money(1000, 'USD'), 'Deposit 1');
      transaction1.complete();

      const transaction2 = OptimizedTransaction.createWithdrawal('TXN-002', 'acc_2', 'user_2', new Money(500, 'USD'), 'Withdrawal 1');
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.getStatistics()).toBeRejectedWithError(RepositoryError, '獲取交易統計資料失敗');
    });
  });
  ...
  describe('getAccountStatistics', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.getAccountStatistics(accountId)).toBeRejectedWithError(RepositoryError, '獲取帳戶交易統計資料失敗');
    });
  });
  ...
  describe('getUserStatistics', () => {
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.getUserStatistics(userId)).toBeRejectedWithError(RepositoryError, '獲取用戶交易統計資料失敗');
    });
  });
  ...
  describe('getTransactionHistoryReport', () => {
    it('should generate transaction history report correctly', async () => {
      ...
      const transaction1 = OptimizedTransaction.createDeposit('TXN-001', accountId, 'user_1', new Money(1000, 'USD'), 'Deposit 1');
      transaction1.complete();
      transaction1.category = '存款';

      const transaction2 = OptimizedTransaction.createWithdrawal('TXN-002', accountId, 'user_1', new Money(500, 'USD'), 'Withdrawal 1');
    ...
    it('should throw RepositoryError on failure', async () => {
      ...
      // Act & Assert
      await expectAsync(repository.getTransactionHistoryReport(startDate, endDate)).toBeRejectedWithError(
        RepositoryError,
        '獲取交易歷史報表失敗'
      );
    });
