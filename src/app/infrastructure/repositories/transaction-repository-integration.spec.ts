...
describe('Transaction Repository Integration', () => {
    ...
    describe('Performance and Reliability', () => {
        it('should handle large number of transactions', async () => {
            ...
            for (let i = 0; i < 100; i++) {
                const transaction = OptimizedTransaction.create(
                    ...
                    i % 3 === 0 ? TransactionType.DEPOSIT :
                        i % 3 === 1 ? TransactionType.WITHDRAWAL : TransactionType.TRANSFER,
                    `Transaction ${i}`
                );
        ...
        it('should handle concurrent operations', async () => {
            ...
            // 並行操作
            await Promise.all([
                mockRepository.save(transaction1),
                mockRepository.save(transaction2)
            ]);

            const [found1, found2] = await Promise.all([
                mockRepository.findById(transaction1.id),
                mockRepository.findById(transaction2.id)
            ]);
