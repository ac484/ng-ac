...
describe('Account Repository Integration', () => {
    ...
    describe('Performance and Reliability', () => {
        it('should handle large number of accounts', async () => {
            ...
            const accounts = [];
            for (let i = 0; i < 100; i++) {
                accounts.push(OptimizedAccount.create({
                    userId: `user_${i % 10}`,
                    accountNumber: `ACC-${String(i).padStart(3, '0')}`,
                    name: `Account ${i}`,
                    type: i % 3 === 0 ? AccountType.CHECKING :
                        i % 3 === 1 ? AccountType.SAVINGS : AccountType.CREDIT,
        ...
        it('should handle concurrent operations', async () => {
            ...
            // 並行操作
            await Promise.all([
                mockRepository.save(account1),
                mockRepository.save(account2)
            ]);

            const [found1, found2] = await Promise.all([
                mockRepository.findById(account1.id),
                mockRepository.findById(account2.id)
            ]);
