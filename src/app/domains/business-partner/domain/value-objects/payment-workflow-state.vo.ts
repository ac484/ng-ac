...
export class PaymentWorkflowState extends ValueObject<{
    ...
    static create(
        ...
        const props = {
            ...
            stateHistory: stateHistory.length > 0 ? stateHistory : [{
                state: currentState,
                timestamp: new Date(),
                operator: 'System',
    ...
    private static getAvailableTransitions(currentState: PaymentWorkflowStateEnum): PaymentWorkflowStateEnum[] {
        const transitions: Record<PaymentWorkflowStateEnum, PaymentWorkflowStateEnum[]> = {
            [PaymentWorkflowStateEnum.Draft]: [
                PaymentWorkflowStateEnum.Submitted,
                PaymentWorkflowStateEnum.Cancelled
            ],
            [PaymentWorkflowStateEnum.Submitted]: [
                PaymentWorkflowStateEnum.Reviewing,
                PaymentWorkflowStateEnum.Cancelled
            ],
            [PaymentWorkflowStateEnum.Reviewing]: [
                PaymentWorkflowStateEnum.Approved,
                PaymentWorkflowStateEnum.Rejected
            ],
            [PaymentWorkflowStateEnum.Approved]: [
                PaymentWorkflowStateEnum.Processing
            ],
            [PaymentWorkflowStateEnum.Rejected]: [
                PaymentWorkflowStateEnum.Draft
            ],
