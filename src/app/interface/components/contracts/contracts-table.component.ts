...
@Component({
    ...
    <div class="table-container">
      <table mat-table [dataSource]="contracts()" class="contracts-table">
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let contract">
            <div class="actions">
              <button mat-icon-button (click)="viewContract(contract)"><mat-icon>visibility</mat-icon></button>
              <button mat-icon-button (click)="editContract(contract)"><mat-icon>edit</mat-icon></button>
              <button mat-icon-button (click)="exportContract(contract)"><mat-icon>download</mat-icon></button>
            </div>
          </td>
        </ng-container>

        <tr mat-header-row *matHeaderRowDef="displayedColumns"></tr>
        <tr mat-row *matRowDef="let row; columns: displayedColumns;" class="contract-row" (click)="viewContract(row)"></tr>
      </table>
  ...
  styles: [`
    ...
    .contracts-table{width:100%;min-width:800px}
    .contract-row{cursor:pointer;transition:background-color .2s ease}
    .contract-row:hover{background-color:var(--mat-sys-surface-variant)}
    .contract-title{display:flex;flex-direction:column;gap:4px}
    .contract-title .title{font-weight:500}
    .contract-title .contractor{font-size:.875rem;color:var(--mat-sys-on-surface-variant)}
    .date-range{display:flex;flex-direction:column;gap:2px;font-size:.875rem}
    .actions{display:flex;gap:4px}
    .empty-state{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;text-align:center;color:var(--mat-sys-on-surface-variant)}
    ...
    @media (max-width:768px){.contracts-table{min-width:600px}.date-range{font-size:.75rem}}
  `]
})
