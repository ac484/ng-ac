...
@Component({
    ...
    <form nz-form 
          [formGroup]="form" 
          [nzLayout]="config.layout || 'vertical'"
          [nzAutoTips]="autoTips">
      
      <nz-row [nzGutter]="16">
        <nz-col *ngFor="let field of visibleFields" 
                [nzSpan]="field.span || getDefaultSpan()">
          
          <nz-form-item>
            <nz-form-label 
              [nzRequired]="field.required"
              [nzSpan]="config.layout === 'horizontal' ? (config.labelSpan || 6) : null">
              {{ field.label }}
            </nz-form-label>
            ...
            <nz-form-control 
              ...
              <!-- 單選按鈕組 -->
              <nz-radio-group *ngIf="field.type === 'radio'"
                              [formControlName]="field.key"
                              [nzDisabled]="field.disabled || false">
...
export class DynamicFormComponent implements OnInit, OnDestroy {
    ...
    /** 自動提示配置 */
    autoTips: { [key: string]: { [key: string]: string } } = {
        'zh-tw': {
            required: '此欄位為必填項目',
            email: '請輸入有效的電子郵件地址',
