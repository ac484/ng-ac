...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NzCardModule, NzDividerModule, DynamicFormComponent],
  template: `
    <div style="padding: 24px;">
      ...
      <nz-card nzTitle="水平佈局表單" style="margin-bottom: 24px;">
        <app-dynamic-form [config]="horizontalFormConfig" [loading]="horizontalLoading" (formSubmit)="onHorizontalSubmit($event)">
        </app-dynamic-form>
      </nz-card>

      <!-- 行內佈局表單範例 -->
      <nz-card nzTitle="行內佈局表單">
        <app-dynamic-form [config]="inlineFormConfig" [showResetButton]="false" submitText="搜尋" (formSubmit)="onInlineSubmit($event)">
        </app-dynamic-form>
      </nz-card>
    </div>
  `
    ...
    fields: [
      {
        ...
        required: true,
        placeholder: '請輸入姓名',
        validators: [{ type: 'minLength', value: 2, message: '姓名至少需要2個字符' }]
      },
      {
        key: 'email',
        label: '電子郵件',
      ...
      },
      {
        ...
        type: 'text',
        placeholder: '請輸入電話號碼',
        validators: [{ type: 'pattern', value: '^[0-9-+()\\s]+$', message: '請輸入有效的電話號碼' }]
      }
    ],
    layout: 'vertical',
    autoFocus: true
  ...
  advancedFormConfig: FormConfig = {
    fields: [
      ...
      },
      {
        ...
        required: true,
        placeholder: '請輸入密碼',
        validators: [{ type: 'minLength', value: 8, message: '密碼至少需要8個字符' }],
        span: 12
      },
      {
        key: 'role',
  ...
  horizontalFormConfig: FormConfig = {
    fields: [
      ...
      },
      {
        ...
        validators: [{ type: 'pattern', value: '^https?://.+', message: '請輸入有效的網址' }]
      }
    ],
    layout: 'horizontal',
    labelSpan: 6,
