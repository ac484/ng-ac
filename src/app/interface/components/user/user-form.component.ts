...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NzButtonModule, NzCardModule, DynamicFormComponent],
  template: `
...
export class UserFormComponent implements OnInit {
  ...
  private initFormConfig(): void {
    const baseFields: FormField[] = [
      {
        ...
        required: true,
        placeholder: '請輸入顯示名稱',
        validators: [{ type: 'maxLength', value: 100, message: '顯示名稱不能超過100個字符' }]
      },
      {
        key: 'phoneNumber',
        label: '電話號碼',
    ...
    if (!this.isEdit) {
      ...
      baseFields.unshift({
        ...
        required: true,
        placeholder: '請輸入郵箱地址',
        validators: [{ type: 'email', message: '請輸入有效的郵箱地址' }]
      });
      baseFields.push({
        key: 'password',
        label: '密碼',
        ...
        required: true,
        placeholder: '請輸入密碼',
        validators: [{ type: 'minLength', value: 6, message: '密碼至少需要6個字符' }]
      });
    } else {
      // 編輯模式需要狀態選擇
      baseFields.push({
  ...
  onSubmit(formValue: any): void {
    ...
    if (this.isEdit) {
      ...
      this.userApplicationService
        .updateUserProfile(this.userId!, updateDto)
        .then(() => {
