...
describe('UserApplicationService (Refactored with BaseApplicationService)', () => {
    ...
    beforeEach(() => {
        const userRepositorySpy = jasmine.createSpyObj('UserRepository', [
            'findById', 'findByEmail', 'findAll', 'save', 'delete'
        ]);
        const userDomainServiceSpy = jasmine.createSpyObj('UserDomainService', [
            'createUser', 'updateUserProfile', 'updateUserStatus'
        ]);
        const conversionUtilitiesSpy = jasmine.createSpyObj('ConversionUtilitiesService', [
            'stringToUserStatus'
        ]);
    ...
    describe('createUser (using base class)', () => {
        ...
        it('should handle user already exists error', async () => {
            ...
            await expectAsync(service.createUser(createUserDto))
                .toBeRejected();
        });
    });
    ...
    describe('updateUserProfile (using base class)', () => {
        it('should update user profile successfully using base class method', async () => {
            ...
            expect(result).toBeDefined();
            expect(mockUserDomainService.updateUserProfile).toHaveBeenCalledWith(
                mockUser,
                'Updated Name',
                undefined
            );
    ...
    describe('updateUserStatus', () => {
        ...
        it('should handle user not found error', async () => {
            ...
            await expectAsync(service.updateUserStatus('non-existent', updateStatusDto))
                .toBeRejected();
        });
    });
    ...
    describe('filtering and sorting (base class methods)', () => {
        ...
        it('should sort users by email', async () => {
            const users = [
                createTestUser({ email: 'zebra@example.com' }),
                createTestUser({ email: 'alpha@example.com' })
            ];
