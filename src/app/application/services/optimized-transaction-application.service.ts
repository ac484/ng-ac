...
> {
  constructor(@Inject(TRANSACTION_REPOSITORY) repository: Repository<OptimizedTransaction>, errorHandler: ErrorHandlerService) {
  ...
  protected override async applySearchCriteria(
    ...
    if (transactionCriteria.referenceNumber) {
      filtered = filtered.filter(entity => entity.referenceNumber?.includes(transactionCriteria.referenceNumber!));
    }

    // 分類過濾
    if (transactionCriteria.category) {
      filtered = filtered.filter(entity => entity.category?.includes(transactionCriteria.category!));
    }
