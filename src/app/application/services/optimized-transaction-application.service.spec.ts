...
import { OptimizedTransactionApplicationService, CreateTransactionDto, UpdateTransactionDto, ProcessTransactionDto } from './optimized-transaction-application.service';
...
describe('OptimizedTransactionApplicationService', () => {
    ...
    beforeEach(() => {
        // 創建 mock 物件
        mockRepository = jasmine.createSpyObj('Repository', [
            'findById',
            'findAll',
            'save',
            'delete'
    ...
    describe('create', () => {
        ...
        it('should throw ValidationError when accountId is empty', async () => {
            ...
            // Act & Assert
            await expectAsync(service.create(createDto))
                .toBeRejectedWithError(ValidationError, '帳戶 ID 不能為空');
        });
        ...
        it('should throw ValidationError when amount is zero or negative', async () => {
            ...
            // Act & Assert
            await expectAsync(service.create(createDto))
                .toBeRejectedWithError(ValidationError, '交易金額必須大於零');
        });
        ...
        it('should throw ValidationError when description is empty', async () => {
            ...
            // Act & Assert
            await expectAsync(service.create(createDto))
                .toBeRejectedWithError(ValidationError, '交易描述不能為空');
        });
    ...
    describe('update', () => {
        ...
        it('should throw NotFoundError when transaction does not exist', async () => {
            ...
            // Act & Assert
            await expectAsync(service.update(transactionId, updateDto))
                .toBeRejectedWithError(NotFoundError, '實體不存在');
        });
    });
    ...
    describe('updateStatus', () => {
        ...
        it('should throw ValidationError for invalid status', async () => {
            ...
            // Act & Assert
            await expectAsync(service.updateStatus(transactionId, statusDto))
                .toBeRejectedWithError(ValidationError, '無效的交易狀態');
        });
    });
    ...
    describe('processTransaction', () => {
        ...
        it('should throw ValidationError for invalid action', async () => {
            ...
            // Act & Assert
            await expectAsync(service.processTransaction(transactionId, processDto))
                .toBeRejectedWithError(ValidationError, '不支援的處理動作: invalid_action');
        });
    });
    ...
    describe('addFees', () => {
        ...
        it('should throw ValidationError when fee amount is zero or negative', async () => {
            ...
            // Act & Assert
            await expectAsync(service.addFees(transactionId, feeAmount))
                .toBeRejectedWithError(ValidationError, '手續費金額必須大於零');
        });
    });
    ...
    describe('getByAccountId', () => {
        ...
        it('should throw ValidationError when account ID is empty', async () => {
            // Act & Assert
            await expectAsync(service.getByAccountId(''))
                .toBeRejectedWithError(ValidationError, '帳戶 ID 不能為空');
        });
    });
    ...
    describe('getByUserId', () => {
        ...
        it('should throw ValidationError when user ID is empty', async () => {
            // Act & Assert
            await expectAsync(service.getByUserId(''))
                .toBeRejectedWithError(ValidationError, '用戶 ID 不能為空');
        });
    });
    ...
    describe('delete', () => {
        ...
        it('should throw ValidationError when trying to delete completed transaction', async () => {
            ...
            // Act & Assert
            await expectAsync(service.delete(transactionId))
                .toBeRejectedWithError(ValidationError, '已完成的交易不能刪除');
        });
    });
