...
export class OptimizedUserApplicationService extends BaseApplicationService<OptimizedUser, CreateUserDto, UpdateUserDto, UserResponseDto> {
  constructor(@Inject(USER_REPOSITORY) repository: Repository<OptimizedUser>, errorHandler: ErrorHandlerService) {
  ...
  protected override async filterByStatus(entities: OptimizedUser[], status: string): Promise<OptimizedUser[]> {
    switch (status.toLowerCase()) {
      ...
      case 'active':
        ...
        return entities.filter(user => user.lastLoginAt && user.lastLoginAt > thirtyDaysAgo);
      default:
  ...
  async getUserStats(): Promise<{
    ...
    try {
      ...
      const stats = {
        ...
        active: allUsers.filter(u => u.lastLoginAt && u.lastLoginAt > thirtyDaysAgo).length
      };
