...
import {
    ...
    UpdateUserDto,
    UserResponseDto
} from './optimized-user-application.service';
...
describe('OptimizedUserApplicationService', () => {
    ...
    describe('create', () => {
        ...
        it('當顯示名稱為空時應該拋出錯誤', async () => {
            ...
            // Act & Assert
            await expectAsync(service.create(createDto))
                .toBeRejectedWithError('顯示名稱不能為空');
        });
        ...
        it('當顯示名稱過長時應該拋出錯誤', async () => {
            ...
            // Act & Assert
            await expectAsync(service.create(createDto))
                .toBeRejectedWithError('顯示名稱不能超過 50 個字元');
        });
    });
    ...
    describe('update', () => {
        ...
        it('當更新的顯示名稱為空時應該拋出錯誤', async () => {
            ...
            // Act & Assert
            await expectAsync(service.update('test-user-id', updateDto))
                .toBeRejectedWithError('顯示名稱不能為空');
        });
    });
    ...
    describe('getUserByEmail', () => {
        ...
        it('當郵箱格式無效時應該拋出錯誤', async () => {
            // Act & Assert
            await expectAsync(service.getUserByEmail('invalid-email'))
                .toBeRejectedWithError('無效的郵箱地址');
        });
    });
    ...
    describe('verifyUserEmail', () => {
        ...
        it('當用戶不存在時應該拋出錯誤', async () => {
            ...
            // Act & Assert
            await expectAsync(service.verifyUserEmail('non-existent-id'))
                .toBeRejectedWithError('用戶不存在');
        });
    });
    ...
    describe('updateLastLogin', () => {
        ...
        it('當用戶不存在時不應該拋出錯誤（只記錄日誌）', async () => {
            ...
            // Act - 不應該拋出錯誤
            await expectAsync(service.updateLastLogin('non-existent-id'))
                .toBeRejectedWithError('用戶不存在');
        });
    });
    ...
    describe('getList with filters', () => {
        it('應該正確過濾已驗證用戶', async () => {
            // Arrange
            const users = [
                createTestUser({ id: 'user1', isEmailVerified: true }),
                createTestUser({ id: 'user2', isEmailVerified: false })
            ];
