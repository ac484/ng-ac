...
> {
    ...
    constructor(
        @Inject('ACCOUNT_REPOSITORY') repository: Repository<OptimizedAccount>,
        ...
        if (criteria && this.isAccountSearchCriteria(criteria)) {
            ...
            if (accountCriteria.accountNumber) {
                filtered = filtered.filter(account =>
                    account.accountNumber.toLowerCase().includes(accountCriteria.accountNumber!.toLowerCase())
                );
            }

            if (accountCriteria.accountName) {
                filtered = filtered.filter(account =>
                    account.name.toLowerCase().includes(accountCriteria.accountName!.toLowerCase())
                );
            }
