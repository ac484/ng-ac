...
export class GlobalErrorHandler implements ErrorHandler {
    ...
    // 錯誤去重機制
    private errorDeduplication = new Map<string, {
        count: number;
        lastOccurrence: Date;
    ...
    private showUserMessage(error: BaseError, severity: ErrorSeverity): void {
        this.ngZone.run(() => {
            ...
            const duration = severity === ErrorSeverity.CRITICAL ?
                this.CRITICAL_ERROR_DURATION : this.ERROR_DISPLAY_DURATION;
    ...
    private cleanupExpiredDeduplication(): void {
        ...
        for (const [key, data] of this.errorDeduplication.entries()) {
            ...
            if (timeDiff > this.DEDUPLICATION_WINDOW * 2) { // 保留兩倍的去重視窗時間
