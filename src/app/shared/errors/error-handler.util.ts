...
export class ErrorHandlerUtil {
  ...
  static toBaseError(error: any, context?: Record<string, any>): BaseError {
    ...
    if (error instanceof Error) {
      return new ApplicationError(error.message || '未知錯誤', undefined, error, context);
    }
    ...
    return new ApplicationError('發生未知錯誤', undefined, undefined, { ...context, originalError: error });
  }

  /**
   * 根據錯誤碼創建對應的錯誤
   */
  static createErrorByCode(code: ErrorCode, message?: string, context?: Record<string, any>): BaseError {
   ...
   * 從 HTTP 錯誤回應創建錯誤
   */
  static fromHttpError(status: number, statusText: string, url?: string, method?: string, responseBody?: any): BaseError {
    const context = {
      status,
      statusText,
      url,
   ...
   * 檢查是否為特定類型的錯誤
   */
  static isErrorType<T extends BaseError>(error: any, errorClass: new (...args: any[]) => T): error is T {
   ...
   * 安全地執行可能拋出錯誤的函數
   */
  static async safeExecute<T>(fn: () => Promise<T>, fallback?: T, onError?: (error: BaseError) => void): Promise<T | undefined> {
    try {
      return await fn();
    } catch (error) {
      const baseError = this.toBaseError(error);
   ...
   * 同步版本的安全執行
   */
  static safeExecuteSync<T>(fn: () => T, fallback?: T, onError?: (error: BaseError) => void): T | undefined {
    try {
      return fn();
    } catch (error) {
      const baseError = this.toBaseError(error);
 ...
 * 錯誤處理裝飾器
 */
export function HandleError(fallback?: any, logError = true) {
 ...
 * 同步方法錯誤處理裝飾器
 */
export function HandleErrorSync(fallback?: any, logError = true) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    const method = descriptor.value;
