...
@Component({
    ...
    <nz-card nzTitle="錯誤處理示範" [nzBordered]="false">
      ...
      <nz-space nzDirection="vertical" [nzSize]="16" style="width: 100%;">
        <nz-space>
          <button nz-button nzType="default" (click)="triggerValidationError()">
            驗證錯誤 (低嚴重程度)
          </button>
          <button nz-button nzType="default" (click)="triggerAuthError()">
            認證錯誤 (中等嚴重程度)
          </button>
          <button nz-button nzType="default" (click)="triggerNotFoundError()">
            資源不存在錯誤 (低嚴重程度)
          </button>
        </nz-space>
        
        <nz-space>
          <button nz-button nzType="default" (click)="triggerNetworkError()">
            網路錯誤 (中等嚴重程度)
          </button>
          <button nz-button nzType="default" (click)="triggerServerError()">
            伺服器錯誤 (高嚴重程度)
          </button>
          <button nz-button nzType="default" (click)="triggerApplicationError()">
            應用程式錯誤 (中等嚴重程度)
          </button>
        </nz-space>
      </nz-space>
      ...
      <nz-space>
        <button nz-button nzType="dashed" (click)="triggerDuplicateError()">
          觸發重複錯誤 (測試去重功能)
        </button>
        <button nz-button nzType="dashed" (click)="showErrorStats()">
          顯示錯誤統計
        </button>
        <button nz-button nzType="dashed" (click)="clearErrorStats()">
          清除錯誤統計
        </button>
      </nz-space>
      ...
      <nz-space>
        <button nz-button nzType="dashed" (click)="testRetryableError()">
          可重試錯誤
        </button>
        <button nz-button nzType="dashed" (click)="testNonRetryableError()">
          不可重試錯誤
        </button>
      </nz-space>
    </nz-card>
...
export class ErrorDemoComponent {
    ...
    showErrorStats(): void {
        ...
        this.globalErrorHandler.handleInfo(
            `錯誤統計：總計 ${stats.totalErrors} 個錯誤，最近 ${stats.recentErrors.length} 個錯誤`
        );
    }
    ...
    testRetryableError(): void {
        ...
        this.globalErrorHandler.handleInfo(
            `此錯誤${isRetryable ? '可以' : '不可以'}重試，建議延遲 ${retryDelay}ms`
        );
    }
