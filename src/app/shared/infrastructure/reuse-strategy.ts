...
import { getDeepReuseStrategyKeyFn, fnGetReuseStrategyKeyFn } from '../application/utils/tools';
...
// Simple scroll service for our project
class SimpleScrollService {
    getScrollPosition(el?: Element): [number, number] {
        if (el) {
    ...
    scrollToPosition(el: Element | null, position: [number, number]): void {
        if (el) {
...
// Simple theme service for our project
class SimpleThemeService {
    private isShowTab$ = new BehaviorSubject({ isShowTab: true });

    getThemesMode() {
        return this.isShowTab$.asObservable();
    }
}
...
export class SimpleReuseStrategy implements RouteReuseStrategy {
    ...
    private readonly scrollService = new SimpleScrollService();
    private readonly themeService = new SimpleThemeService();

    // 缓存每个component的map
    static handlers: { [key: string]: any } = {};
    // 缓存每个页面的scroll位置
    static scrollHandlers: { [key: string]: any } = {};
    ...
    // 是否有多页签，没有多页签则不做路由缓存
    isShowTab$ = this.themeService.getThemesMode();
    ...
    shouldDetach(route: ActivatedRouteSnapshot): boolean {
        ...
        let isShowTab = true; // Default to true for our project
